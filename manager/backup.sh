...
if [ ! -d "$DEBIAN_DIR" ]; then
    dialog --no-shadow --msgbox "$label_error: $DEBIAN_DIR $label_not_found" $dialog_height $dialog_width
    exit 1
fi
...
if [ -z "$SENHA" ]; then
    dialog --no-shadow --msgbox "$label_andistro_backup_password_empty" $dialog_height $dialog_width
    exit 1
fi
...
show_progress_dialog steps-multi-label-alt 5 \
    "${label_andistro_backup_starting}" "echo \"[1/5] Iniciando backup\" >> \"$LOG_FILE\"; sleep 2" \
    "${label_andistro_backup_compacting} (${VERSION_CODENAME})...\n\n${label_step_slow}" "echo \"[2/5] Compactando sistema Debian (stable)\" >> \"$LOG_FILE\"; tar -czf \"$TAR_TEMP\" -C \"$DEBIAN_DIR\" . >> \"$LOG_FILE\" 2>&1" \
    "${label_andistro_backup_encrypting}" "echo \"[3/5] Criptografando com GPG\" >> \"$LOG_FILE\"; gpg --batch --yes --passphrase-file \"$SENHA_FILE\" --symmetric --cipher-algo AES256 --output \"$GPG_TEMP\" \"$TAR_TEMP\" >> \"$LOG_FILE\" 2>&1" \
    "${label_andistro_backup_verifying}" "echo \"[4/5] Verificando arquivo $GPG_TEMP\" >> \"$LOG_FILE\"; if [ -f \"$GPG_TEMP\" ]; then echo \"OK: $GPG_TEMP\" >> \"$LOG_FILE\"; else echo \"ERRO: arquivo não encontrado\" >> \"$LOG_FILE\"; fi; sleep 2" \
    "${label_andistro_backup_verifying}" "echo \"[5/5] Limpando temporários\" >> \"$LOG_FILE\"; rm -f \"$TAR_TEMP\" \"$SENHA_FILE\"; sleep 2"
...
if [ -f "$ARQ" ]; then
    dialog --no-shadow --msgbox "$label_andistro_backup_success\n\n$label_file: $ARQ\n$label_size: $(du -sh "$ARQ" | cut -f1)" $dialog_height $dialog_width
    exit 0
else
    dialog --no-shadow --msgbox "$label_andistro_backup_error\n$label_view_log:\n$LOG_FILE" $dialog_height $dialog_width
    exit 1
fi
